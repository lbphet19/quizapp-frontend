...
export class CategoryViewExamComponent implements OnInit {
  ...
  initForm() {
    this.examUpdateForm = this.formBuilder.group({
      ...
      examImage: new FormControl(
        'https://cdn.pixabay.com/photo/2021/02/03/05/37/question-mark-5976736_960_720.png'
      ),
  ...
  updateModal() {
    ...
    this.examService.put(val).subscribe(
      (ex) => {
        location.reload();
      },
      (err) => alert(err)
  ...
  viewScore(exam:any){
    this.router.navigate(['exam', exam.examId, 'score']);
  }
}
